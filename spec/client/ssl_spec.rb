...
describe 'ssl' do
  ...
  skip 'should fail to connect to an ssl server without TLS/SSL negotiation' do
    expect do
      NATS.start(:uri => TEST_SERVER_SSL) { NATS.stop }
    end.to raise_error NATS::Error
  end

  skip 'should fail to connect to an no ssl server with TLS/SSL negotiation' do
    expect do
      NATS.start(:uri => TEST_SERVER_NO_SSL, :ssl => true) { NATS.stop }
    end.to raise_error NATS::Error
  end

  skip 'should run TLS/SSL negotiation' do
    expect do
      NATS.start(:uri => TEST_SERVER_SSL, :ssl => true) { NATS.stop }
    end.to_not raise_error
  end

  skip 'should not run TLS/SSL negotiation' do
    expect do
